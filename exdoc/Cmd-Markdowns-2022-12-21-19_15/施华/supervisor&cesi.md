...
# **supervisor**
+ 安装
https://www.jianshu.com/p/cb25c5648019
https://www.cnblogs.com/garvenc/p/supervisor_install_configure.html
...
$cp echo_supervisord_conf /usr/bin
```
+ 配置
```
...
+ 问题
    ...
    ```
    + yaml问题，由于pyyaml6.0以上对安全要求更高，所以需要降低yaml的版本，比如5.1
...
```
+ 组管理：例如，SHcloud——meteorological项目配置，在supervisord.conf配置文件中修改[group:SHcloud_meteorological]下相关参数,并将同一个组别下的目标进程统一到一个SHcloud_meteorological.conf下进行管理
```

[group:SHcloud_meteorological]
programs=SHcloud_weather_API,SHcloud_nwp_7day_API,satellite_resource_API,NWP4d_satellite_GZ_API  ; each refers to 'x' in [program:x] definitions
priority=999                  ; the relative start priority (default 999)
```
+ SHcloud_meteorological.conf文件实例：
```
[group:SHcloud_meteorological]
programs = SHcloud_weather_API,Shcloud_nwp_7day_API,NWP4d_satellite_GZ_API,satellite_resource_API


[program:SHcloud_weather_API]
directory=/root/
command=/root/anaconda3/bin/python /home/SHcloud/weather/SHcloud_weather_API.py
autostart=true
autorestart=true
startsecs=1
user=root
stderr_logfile=/root/logs/SHcloud_weather_API_stderr.log
stdout_logfile=/root/logs/SHcloud_weather_API_stdout.log
redirect_stderr=true
stdout_logfile_maxbytes = 20MB
stdout_logfile_backups = 20


[program:SHcloud_nwp_7day_API]
directory=/root/
command=/root/anaconda3/bin/python /home/SHcloud/NWP_forecast/SHcloud_nwp_7day_API.py
autostart=true
autorestart=true
startsecs=1
user=root
stderr_logfile=/root/logs/SHcloud_NWP_7day_API_stderr.log
stdout_logfile=/root/logs/SHcloud_NWP_7day_API_stdout.log
redirect_stderr=true
stdout_logfile_maxbytes = 20MB
stdout_logfile_backups = 20


[program:NWP4d_satellite_GZ_API]
directory=/root/
command=/root/anaconda3/bin/python /home/SHcloud/NWP_forecast/NWP4d_satellite_GZ_API.py
autostart=true
autorestart=true
startsecs=1
user=root
stderr_logfile=/root/logs/NWP4d_satellite_GZ_API_stderr.log
stdout_logfile=/root/logs/NWP4d_satellite_GZ_API_stdout.log
redirect_stderr=true
stdout_logfile_maxbytes = 20MB
stdout_logfile_backups = 20


[program:satellite_resource_API]
directory=/root/
command=/root/anaconda3/bin/python /home/SHcloud/satellite/satellite_resource_API.py
autostart=true
autorestart=true
startsecs=1
user=root
stderr_logfile=/root/logs/satellite_resource_API_stderr.log
stdout_logfile=/root/logs/satellite_resource_API_stdout.log
redirect_stderr=true
stdout_logfile_maxbytes = 20MB
stdout_logfile_backups = 20
...
```
# **systemd管理开机自启**
+ 新建supervisord.service,/lib/systemd/system
```
[Unit]
...
ExecReload=/home/shihua/anaconda3/bin/supervisorctl reload
KillMode=process
Restart=on-failure
RestartSec=42s
...
![cesi.PNG-107.6kB][1]
+ supervisor
![supervisor.PNG-78.9kB][2]


  [1]: http://static.zybuluo.com/tulip0216/ashoybq2zdhwptv4hnj7ddsz/cesi.PNG
  [2]: http://static.zybuluo.com/tulip0216/ewf8h4rrw8poj98cj9avnyx8/supervisor.PNG